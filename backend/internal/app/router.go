...
func NewRouter(container *Container, env string) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		api.GET("/schedule", container.ScheduleHandler.GetSchedule)
		api.PUT("/schedule", container.ScheduleHandler.UpsertSchedule)
		api.PATCH("/schedule/days", container.ScheduleHandler.UpsertDaySchedules)
		api.PUT("/schedule/exceptions", container.ScheduleHandler.UpsertDateExceptions)
		api.DELETE("/schedule/exceptions/:date", container.ScheduleHandler.DeleteDateException)
